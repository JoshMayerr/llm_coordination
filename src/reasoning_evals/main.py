...
# from dataclasses import dataclass
# import re
# from llm_coordination_agents.LanguageAgent import LLMAgent
...
# class TestLLMCoordination:
#     def __init__(self, df, game_name, engine_params, num_trials=3, eval_options='all'):
#         self.game_name = game_name 
#         self.df = df 
#         self.eval_options = eval_options.lower()
#         self.llm_agent = LLMAgent(engine_params=engine_params)
#         self.num_trials = num_trials
#         self.issues = []
#         self.log = {'EC_ANSWERS': [], 'TOM_ANSWERS': [], 'JP_ANSWERS': []}
#         self.answer_log = []
#         logging.info(f"Initialized TestLLMCoordination with {len(self.df)} scenarios")
...
#         scenario_row = self.df.iloc[idx]
#         current_game = scenario_row['Game']
#         scenario = EvalDataPoint(
#             concept=scenario_row['Concept'],
#             game_desc=scenario_row['Game Description'],
#             ec_directive=scenario_row['EC Directive'],
#             tom_directive=scenario_row['TOM Directive'],
#             jp_directive=scenario_row['JP Directive'],
...
#         if self.eval_options in ['all', 'ec']:
#             logging.info(f"Processing EC for scenario {idx}")
#             self.llm_agent.reset()
#             self.llm_agent.add_system_prompt(scenario.ec_directive)
#             self.llm_agent.add_message(scenario.game_desc)
#             self.llm_agent.add_message('Got it!')
#             try:
#                 self.llm_agent.add_message(scenario.ec_question)
#                 ec_inference = self.llm_agent.get_response(temperature=0.7)
#                 ec_answer_llm = self.find_answer_fuzzy(scenario.ec_question, ec_inference)
#                 scores.append(1 if ec_answer_llm in scenario.ec_answer_ordinal.split(',') else 0)
#                 self.log['EC_ANSWERS'].append(ec_inference)
#                 self.answer_log.append({'type': 'EC', 'question': scenario.ec_question, 'response': ec_inference})
#             except Exception as e:
#                 logging.error(f"Error in EC inference for scenario {idx}: {e}")
#                 self.issues.append({'type': 'EC', 'error': str(e), 'game': current_game})

#         if self.eval_options in ['all', 'tom']:
#             logging.info(f"Processing TOM for scenario {idx}")
#             self.llm_agent.reset()
#             self.llm_agent.add_system_prompt(scenario.tom_directive)
#             self.llm_agent.add_message(scenario.game_desc)
#             self.llm_agent.add_message('Got it!')
#             try:
#                 self.llm_agent.add_message(scenario.tom_question)
#                 tom_inference = self.llm_agent.get_response(temperature=0.7)
#                 tom_answer_llm = self.find_answer_fuzzy(scenario.tom_question, tom_inference)
#                 scores.append(1 if tom_answer_llm in scenario.tom_answer_ordinal.split(',') else 0)
#                 self.log['TOM_ANSWERS'].append(tom_inference)
#                 self.answer_log.append({'type': 'TOM', 'question': scenario.tom_question, 'response': tom_inference})
#             except Exception as e:
#                 logging.error(f"Error in TOM inference for scenario {idx}: {e}")
#                 self.issues.append({'type': 'TOM', 'error': str(e), 'game': current_game})
                
#         if self.eval_options in ['all', 'jp']:
#             logging.info(f"Processing JP for scenario {idx}")
#             self.llm_agent.reset()
#             self.llm_agent.add_system_prompt(scenario.jp_directive)
#             self.llm_agent.add_message(scenario.game_desc)
#             self.llm_agent.add_message('Got it!')
#             try:
#                 self.llm_agent.add_message(scenario.jp_question)
#                 jp_inference = self.llm_agent.get_response(temperature=0.7)
#                 jp_answer_llm = self.find_answer_fuzzy(scenario.jp_question, jp_inference)
#                 scores.append(1 if jp_answer_llm in scenario.jp_answer_ordinal.split(',') else 0)
#                 self.log['JP_ANSWERS'].append(jp_inference)
#                 self.answer_log.append({'type': 'JP', 'question': scenario.jp_question, 'response': jp_inference})
#             except Exception as e:
#                 logging.error(f"Error in JP inference for scenario {idx}: {e}")
#                 self.issues.append({'type': 'JP', 'error': str(e), 'game': current_game})    
...
#         game_wise_scores = {'Overcooked': [], 'Hanabi': [], 'CollabGames': []}
#         for t_num in range(self.num_trials):
#             logging.info(f"Starting trial {t_num}")
#             results = []
#             game_wise_results = {'Overcooked': [], 'Hanabi': [], 'CollabGames': []}
#             for sc in tqdm(range(len(self.df))):
#                 res, current_game = self.test_one_scenario(sc)
...
#                 game_wise_scores[game].append(game_wise_score[game])
#             scores.append(score)
#             logging.info(f"Trial {t_num} results: {score}")
#         return {
#             'accuracy': np.mean(np.array(scores), axis=0),
#             'standard error': (np.array(scores).std(axis=0) / np.sqrt(self.num_trials)),
#             'game_wise': game_wise_scores
#         }

#     def format_and_save_results(self, results, model_name, output_file_prefix):
#         """Formats and saves the results and answers to text and JSON files."""
#         timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
#         output_file = f"{output_file_prefix}_{timestamp}_output.txt"
#         answer_log_file = f"{output_file_prefix}_{timestamp}_answers.json"

#         result_str = f"TEST FILE: {self.game_name}\n"
#         result_str += f"MODEL: {model_name}\n"
#         result_str += f"Environment Comprehension Score: {results['accuracy'][0]} +/- {results['standard error'][0]}\n"
#         result_str += f"Theory of Mind Score: {results['accuracy'][1]} +/- {results['standard error'][1]}\n"
#         result_str += f"Joint Planning Score: {results['accuracy'][2]} +/- {results['standard error'][2]}\n\n"

#         for game, game_scores in results['game_wise'].items():
#             game_accuracy = np.mean(game_scores, axis=0)
#             game_std_err = np.std(game_scores, axis=0) / np.sqrt(self.num_trials)
#             result_str += f"{game} Environment Comprehension Score: {game_accuracy[0]} +/- {game_std_err[0]}\n"
#             result_str += f"{game} Theory of Mind Score: {game_accuracy[1]} +/- {game_std_err[1]}\n"
#             result_str += f"{game} Joint Planning Score: {game_accuracy[2]} +/- {game_std_err[2]}\n"
        
#         result_str += f"\nProblems encountered: {self.issues}\n"

#         with open(output_file, 'w') as f:
#             f.write(result_str)
#         with open(answer_log_file, 'w') as f:
#             json.dump(self.answer_log, f, indent=4)

#         logging.info(f"Saved detailed results to {output_file}")
#         logging.info(f"Saved LLM-generated answers to {answer_log_file}")

# if __name__ == '__main__':
#     parser = argparse.ArgumentParser(description="Evaluate LLMs on Coordination Games")
#     parser.add_argument('--model', type=str, required=True, help="Model name to run inference on")
#     parser.add_argument('--eval', type=str, choices=['ec', 'tom', 'jp', 'all'], default='all', help="Choose evaluation type")
#     args = parser.parse_args()

#     engine_type = 'azure' if 'gpt' in args.model else 'vllm'
#     engine_params = {'engine_type': engine_type, 'model': args.model, 'api_version': "2024-08-01-preview"}

#     df = pd.read_csv('data/single_turn_trials.csv')
#     game_name = 'all' if 'Game' not in df else df['Game'].iloc[0]
    
#     evaluator = TestLLMCoordination(df, game_name, engine_params, eval_options=args.eval)
#     results = evaluator.evaluate_llm()
#     evaluator.format_and_save_results(results, args.model, args.model)
